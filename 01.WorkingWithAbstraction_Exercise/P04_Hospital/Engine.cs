...
namespace P04_Hospital
{
    public class Engine
    {
        ...
        {            
            ...
            while (command != "End")
            {
                ...
                if (args.Length == 1)
                {
                    ...
                    PatientInDepartment(departmentName);
                }
        ...
        {
            var allPationsOfDoctor = doctors[fullName]
                .OrderBy(x => x)
                .ToArray();

            Console.WriteLine(string.Join(Environment.NewLine, allPationsOfDoctor));
        }

        private void PrintAllPatientInRoom(int room, string departmentName)
        {
            var allPatientIntRoom = departments[departmentName][room - 1]
                                        .OrderBy(x => x)
                                        .ToArray();
            Console.WriteLine(string.Join(Environment.NewLine, allPatientIntRoom));
        }

        private void PatientInDepartment(string departmentName)
        {
        ...
        private void AddPatientToRoom(string departament, string patient)
        {
            ...
            //int targetRoom = departments[departament].SelectMany(x => x)
            //    .Where(x => x.Count() < 3).FirstOrDefault();
